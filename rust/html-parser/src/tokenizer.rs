...
impl Iterator for Tokenizer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        loop {
            ...
            if reconsume {
                chars = self.document.raw[self.position - self.previous.unwrap().len_utf8()..].chars();
                reconsume = false;
            }
            ...
            match self.state {
                ...
                TokenizationState::TagName => {
                    match current {
                        ...
                        '\u{0041}'..='\u{005A}' => self.tag_name_buf.push(char::to_ascii_lowercase(&current)), // A - Z
                ...
                TokenizationState::AttributeName => {
                    match current {
                        '\u{0009}' | '\u{000A}' | '\u{000C}' | '\u{0020}'=> self.state = TokenizationState::AfterAttributeName, // tab, LF, FF, Space
                        '\u{002F}' => self.state = TokenizationState::SelfClosingStartTag, // /
                        ...
                        '\u{0041}'..='\u{005A}' => {
                            self.attributes_buf.last_mut().unwrap().name.push(char::to_ascii_lowercase(&current));
                        }, // A - Z
                        '\u{0000}' => {
                            self.attributes_buf.last_mut().unwrap().name.push('\u{FFFD}');
                        }, // NULL, Parse error
                ...
                TokenizationState::AttributeValueUnquoted => {
                    match current {
                        '\u{0009}' | '\u{000A}' | '\u{000C}' | '\u{0020}'=> self.state = TokenizationState::BeforeAttributeName, // tab, LF, FF, Space
                        '\u{0026}' => self.state = TokenizationState::CharacterrReferenceInAttributeValue, // & TODO :What is an allowed additional character in a character reference?
                        '\u{003E}' => {
                            self.state = TokenizationState::Data;
                            break;
                        },
                        '\u{0000}' => {
                            self.attributes_buf.last_mut().unwrap().value.push('\u{FFFD}');
                        }, // NULL, Parse error
                        '\u{0022}' | '\u{0027}' | '\u{003C}' | '\u{003D}' | '\u{0060}' => self.state = TokenizationState::BeforeAttributeName, // " ' < = ` Parse error
                        _ => {
                            self.attributes_buf.last_mut().unwrap().value.push(current);
                        }
                        //TODO : EOF
        ...
        let output = if let TagKind::StartTag = self.tag_kind {
            Some(Token::StartTag(self.tag_name_buf.clone(), self.is_self_closing, Vec::from(&mut self.attributes_buf[..])))
        } else {
            Some(Token::EndTag(self.tag_name_buf.clone(), self.is_self_closing, Vec::from(&mut self.attributes_buf[..])))
        };
        self.reset();
        return output;
    }
